...
class MainFrames:
    def __init__(self, main: tk.Tk, header: Frame, mid: Frame, bottom: Frame):
        self._main = main
        self._header = header
        self._mid = mid
        self._footer = bottom
...
class CourseButton(tk.Button):
    ...
    def __init__(self, course: Course, width: int, height: int, applet: MainFrames, user: User):
        self._pixel = tk.PhotoImage(width=1, height=1)
        tk.Button.__init__(self, master=applet.get_mid(), width=width, height=height,
    ...
    def _on_press(self, user_active_courses: List[Course]):
        ...
        if self._course in user_active_courses:
            ...
            user_active_courses.append(self._course)
        print(user_active_courses)
        update_active_courses_footer(user=self._user, applet=self._applet)
...
def create_basic_app() -> tuple:
    ...
    create_basic_frames(root)
    root.update()
    header, midsection, bottom = root.winfo_children()
    applet = MainFrames(main=root, header=header, mid=midsection, bottom=bottom)
    return applet


def create_basic_frames(parent: tk) -> None:
    ...
    # These are static and must be made so
    f = header, unnamed, bottom
    for item in f:
        item.grid_propagate(0)

    header.grid(in_=parent, column=0, row=0)
    unnamed.grid(in_=parent, column=0, row=1)
    bottom.grid(in_=parent, column=0, row=2)


def set_courses_bottom(applet: MainFrames):
    bottom = applet.get_footer()
...
def set_header(applet: MainFrames, user: User):
    header = applet.get_header()
    ...
    set_options_segment(options_segment, user, applet)
    ...
    add_program_button = tk.Button(master=program_entry_frame, text="Add Program", overrelief='raised',
                                   command=lambda: add_program(program_entry, user, programs_segment, applet))

    program_entry.pack()
    add_program_button.pack(side='top', anchor='w')
    program_entry_frame.pack(side='left', padx=5)

    set_programs_segment(programs_segment)


def set_options_segment(options_segment: Frame, user: User, applet: MainFrames):
    ...
    update_button = tk.Button(master=options_segment, text='Update', overrelief='raised',
                              command=lambda: update_credit_num(credit_num_label, user))
    update_button.pack(side='left', padx=5)

    course_entry_frame = tk.Frame(master=options_segment)

    def add_course_from_entry(course_enter: tk.Entry, user: User):
        web_scraping.add_custom_course(course=course_enter.get().upper(), user=user, applet=applet)
    ...
    add_course_button = tk.Button(master=course_entry_frame, text="Add course",
                                  command=lambda: add_course_from_entry(course_enter=course_entry, user=user))
    course_entry.pack()
    add_course_button.pack(side='top', anchor='w')
    course_entry_frame.pack(side='left', padx=5)


def set_programs_segment(programs_segment: Frame):
...
def program_header_update(program_header: Frame, user: User):
...
def update_credit_num(credit_num_frame: tk.Frame, user: User):
    credit_num_frame['text'] = '{0}/20.0'.format(user.get_credits())


def add_program(user_input: tk.Entry, user: User, programs_segment: Frame, applet: MainFrames):
    ...
    program = web_scraping.create_program(user=user, program_code=program_code, applet=applet)
    user.add_program(program)
    program_header_update(programs_segment, user)


def update_active_courses_footer(user: User, applet: MainFrames):
    bot = applet.get_footer()
...
def update_middle_segment(user: User, course: Course, applet: MainFrames):
    parent_frame, bot = applet.get_mid(), applet.get_footer()
